...
figure(1);
semilogy(SNR,BER,'o--b');
hold on;
xlabel('SNR(dB)');
ylabel('BER');
title('SNR Vs BER plot for QAM in Rayleigh fading Channel');
% Rayleigh Theoretical BER
figure(1);
EbN0Lin = 10.^(Eb_No/10);
theoryBerRay = 0.5.*(1-sqrt(EbN0Lin./(EbN0Lin+1)));
semilogy(SNR,theoryBerRay, 'r--o');
% Theoretical BER
figure(1);
theoryBerAWGN = 0.5*erfc(sqrt(10.^(Eb_No/10)));
semilogy(SNR,theoryBerAWGN,'g-+');
legend('Simulated BER', 'Theoretical Raylegh', 'Theoretical AWGN channel');
